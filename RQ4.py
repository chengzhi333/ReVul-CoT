...
def predict_vuln_level(query_code, query_desc, topk_samples):
    ...
    prompt += "Please determine the severity of the above target vulnerability based on the provided samples. Output only the severity level (e.g., LOW, MEDIUM, HIGH, CRITICAL) without any explanation or reasoning.\n"
...
def predict_vuln_level_rag_llm(query_code, query_desc):
    ...
    # 2. 无COT
    level = predict_vuln_level(query_code, query_desc, topk_samples)
    return level
...
if __name__ == "__main__":
    ...
    output_file = "RQ4/test without CoT.xlsx"
    temp_file = "RQ4/test without CoT.xlsx"
